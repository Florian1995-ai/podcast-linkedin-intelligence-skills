...
NAME_STOP=NAME+r"(?=[,)]|\s+(?:of|from|at|on|about|with|has|tells|shares|explains|discusses)\b|\s*[\-:\u2013\u2014|]|\s*$)"
GUEST_PATTERNS=[r"^\s*ep\.?\s*\d+\s*[\-:|]\s*"+NAME+r"(?=,|\s*[\-:\u2013\u2014|])",r"\bwith\s+"+NAME_STOP,r"\bw/\s+"+NAME_STOP,r"\bfeaturing\s+"+NAME_STOP,r"\bfeat\.?\s+"+NAME_STOP,r"\binterview\s+(?:with\s+|of\s+)?"+NAME_STOP,r"^\s*(?:ep\.?\s*\d+\s*[\-:|]\s*)?"+NAME+r"\s*(?:[\-:\u2013\u2014|]|,\s*[A-Z])",r"[\-:\u2013\u2014|]\s*"+NAME+r"\s*$",NAME+r"\s+(?:tells|shares|explains|discusses|talks|reveals)\b"]
SOLO_PATTERNS=[r"^\s*(how to|what is|why|top \d+|\d+ tips|trailer|welcome|bonus|recap|q&a)\b",r"\bsolo\b"]
BAD_NAME_PARTS={'podcast','episode','interview','guest','host','founder','owner','ceo','mobile home','real estate','cash flow','business','marketing','youtube','building','homes','home','prefab','construction','investing'}
def clean_name(name):
